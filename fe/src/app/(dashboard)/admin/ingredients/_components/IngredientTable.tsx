  ...
  return (
    <div className="overflow-x-auto rounded-xl border border-border bg-card">
      <table className="w-full text-sm">
        <thead className="border-b border-border bg-muted text-xs text-muted-fg">
          <tr>
        ...
        <tbody className="divide-y divide-border">
          {ingredients.map((item, idx) => (
            <tr key={item.id} className={`${rowClass(item)} hover:brightness-95 transition-colors`}>
              ...
              <td className="px-4 py-3 text-muted-fg">{item.unit}</td>
              <td className={`px-4 py-3 text-right tabular-nums ${qtyClass(item)}`}>
                {item.quantity}
              </td>
              <td className="px-4 py-3 text-muted-fg">{formatDate(item.importDate)}</td>
              ...
              <td className="px-4 py-3">
                <div className="flex justify-end gap-2">
                  <button
                    onClick={() => onEdit(item)}
                    className="rounded border border-border px-3 py-1 text-xs text-foreground hover:bg-muted min-h-[44px] min-w-[44px]"
                  >
                    Sửa
                  </button>
                  <button
