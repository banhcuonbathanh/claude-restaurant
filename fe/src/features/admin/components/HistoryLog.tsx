...
// updated_at = the moment the order was cancelled or paid
    ...
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-gray-400">Đã huỷ + Đã thanh toán</span>
      ...
      {open && (
        <div className="border-t border-gray-100">
          {isLoading ? (
            ...
            <p className="px-4 py-6 text-center text-sm text-gray-400">Chưa có đơn hoàn thành hoặc bị huỷ hôm nay</p>
          ) : (
            <>
              {/* column headers */}
              <div className="grid grid-cols-[1.5fr_1.5fr_1fr_1fr_1fr_1fr_2fr] gap-2 px-4 py-2 bg-gray-50 border-b border-gray-100 text-xs font-medium text-gray-500 uppercase tracking-wide">
                <span>Bàn</span>
                <span>Mã đơn</span>
              ...
              <div className="divide-y divide-gray-50">
                {orders.map(order => (
                  <div key={order.id}
                    className="grid grid-cols-[1.5fr_1.5fr_1fr_1fr_1fr_1fr_2fr] gap-2 px-4 py-2.5 items-center text-sm hover:bg-gray-50 transition-colors"
                  >
                    <span className="font-medium text-gray-800 truncate">
                      {order.table_name ?? order.table_id ?? '—'}
                    </span>
                    <span className="text-xs font-mono text-gray-400 truncate">{order.order_number}</span>
                    <span className={`inline-flex items-center text-xs font-medium px-2 py-0.5 rounded-full w-fit whitespace-nowrap ${statusColors(order.status)}`}>
                    ...
                    <span className="text-xs text-gray-500 whitespace-nowrap">{fmtDate(order.updated_at)}</span>
