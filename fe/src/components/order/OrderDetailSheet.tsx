...
import {
  X, ChevronDown, ChevronUp, AlertTriangle, CheckCircle, Bell, XCircle, Plus,
} from 'lucide-react'
...
export function OrderDetailSheet({
  ...
  const { order, progress, connectionError, notification, clearNotification } = useOrderSSE(orderId)
  ...
  const cancelOrderMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Đã huỷ đơn hàng'); onClose() },
  ...
  const {
    displayRows, eatenAmount, remainingAmount, totalQty, totalServed,
    comboNameMap, summaryRows, summaryRemainingTotal, summaryGrandTotal,
  } = useMemo(() => {
    if (!order) return {
  ...
  return (
    <>
      ...
      {/* ── Sheet ──────────────────────────────────────────────────────── */}
      <div className="fixed inset-x-0 bottom-0 z-50 max-h-[90dvh] flex flex-col bg-background rounded-t-2xl overflow-hidden shadow-xl">
          ...
          {!order ? (
            ...
            <div className="px-4 pt-4 pb-6 space-y-3">
              ...
              <div className="bg-card rounded-xl overflow-hidden border-l-4 border-primary">
                ...
                >
                  <span className="text-xs text-muted-fg flex-1 text-left">Chi tiết món</span>
                  <span className="text-sm font-bold text-foreground">{formatVND(order.total_amount)}</span>
                  <span className="text-xs font-semibold text-primary">{minutesElapsed(order.created_at)} phút</span>
                  {collapsed
                        ...
                        return (
                          <div key={section.refId} className="border-t border-border/40">
                            ...
                            >
                              ...
                              <span className="text-xs text-muted-fg">{section.items.length} món</span>
                              {isComboCollapsed
              ...
              <div className="bg-card rounded-xl overflow-hidden border-l-4 border-border">
                ...
                {summaryRows.map((row, idx) => (
                  ...
                  >
                    <span className="flex-1 text-sm font-medium text-foreground min-w-0 truncate leading-snug">{row.name}</span>
                    <span className="w-8 text-center font-semibold text-foreground shrink-0">{row.totalQty}</span>
                    <span className="w-8 text-center font-semibold text-success shrink-0">{row.totalServed}</span>
                    ...
                    <span className="w-20 text-right text-muted-fg shrink-0 tabular-nums">{formatVND(row.unitPrice)}</span>
                    <span className="w-20 text-right font-semibold text-foreground shrink-0 tabular-nums">{formatVND(row.totalMoney)}</span>
            ...
            {notification.kind === 'confirmed' ? (
              ...
              <>
                ...
                <div>
                  ...
                  <p className="text-muted-fg text-sm mt-1">Món của bạn sắp được mang ra. Hãy chuẩn bị nhé!</p>
                </div>
              </>
            ) : (
              <>
                ...
                <div>
                  ...
                  <p className="text-muted-fg text-sm mt-1">Nhà hàng đã huỷ đơn của bạn. Vui lòng liên hệ nhân viên.</p>
                </div>
              </>
            )}
            <button
...
function DishRow({
  item, isActive, indent = false, onCancel,
}: {
  item:     OrderItem
  isActive: boolean
  indent?:  boolean
  ...
  return (
    <div className={`flex items-center gap-2 px-4 py-2.5 border-t border-border/40 ${indent ? 'pl-6' : ''}`}>
      ...
      <span className="flex-1 text-sm text-foreground leading-snug min-w-0 truncate">{item.name}</span>
      <div className="flex items-center gap-2 shrink-0 text-xs whitespace-nowrap">
        <span className="text-muted-fg">tổng <span className="text-foreground font-medium">×{item.quantity}</span></span>
        <span className="text-muted-fg">ra <span className="text-success font-medium">×{item.qty_served}</span></span>
        {remaining > 0
          ? <span className="bg-primary/20 text-primary font-semibold px-1.5 py-0.5 rounded">còn ×{remaining}</span>
