...
function roleToRoute(role: User['role']): string {
  switch (role) {
    case 'chef':                 return '/kds'
    case 'cashier': case 'admin': // admin follows manager routing in practice
    case 'customer':              // should never happen for staff login
    case 'manager':               return role === 'cashier' ? '/pos' : '/admin/overview'
    default: return '/pos'
  }
}

// Explicit lookup matching the spec exactly
