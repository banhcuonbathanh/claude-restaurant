...
docs/fe/dev-page/TRACKER.md


Terminal 1 — infra only (run once, rarely restart):


docker compose up -d mysql redis be
...
Now every time you save a file, Next.js reloads in ~1 second instead of Docker rebuilding the image (which takes 30–60s+).

When you DO still need Docker FE rebuild:

Deploying to production
Testing the production build (next build)
Someone else runs the project fresh
When you need to restart the BE (Go code changes):


docker compose up -d --build be
Go builds faster than FE Docker, but if even that's slow, you can also run Go directly:


cd be && go run ./cmd/server
The short answer: remove fe from your daily Docker loop entirely — npm run dev is what it's built for.
