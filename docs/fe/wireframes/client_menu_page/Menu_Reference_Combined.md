...
spec_ref: Spec_3 §4
created: 2026-06-07
updated: 2026-06-07 — menu zones extracted from page.tsx into feature components (branch Refactor-menu); page.tsx is now a pure orchestrator. Only TableConfirmModal remains inline.
...
└── MenuContent                             ← orchestrator: 4 TanStack queries + page state, composes zones
    ...
    └── TableConfirmModal   (modal)         ← still defined INSIDE page.tsx (page.tsx:28-121), QR checkout
```
...
**File:** inline in `page.tsx:28-121` (the **only** zone still inline — extraction candidate) · **Visible:** `confirmOpen === true` (opened by Zone J or CartDrawer)
...
`TableConfirmModal` → `useMutation` → `POST /orders` (page.tsx:34-73). This is the **only** request this page sends.

**Request body** (page.tsx:36-43):
...
| Outcome | Action | Code |
|---|---|---|
| Success | `GET /orders/:id` → cache to `localStorage["order_cache_<id>"]` → `clearCart()` → `router.replace('/order/:id')` | page.tsx:46-61 |
| `TABLE_HAS_ACTIVE_ORDER` | toast "Bàn này đang có đơn chưa hoàn tất…" + `router.replace('/order/<active_order_id>')` | page.tsx:62-69 |
| other error | `toast.error(message ?? 'Đặt hàng thất bại')` | page.tsx:71 |
...
**Handoffs:**
- **Menu → Order:** after POST, the full order is fetched and written to `localStorage["order_cache_<id>"]` (page.tsx:53); `/order/:id` reads that cache. The header "Đơn hàng" dot lights when any `order_cache_` key exists (computed `page.tsx:134-137`, rendered via `MenuHeader`'s `hasOrders` prop).
...
| `TableConfirmModal` | `src/app/(shop)/menu/page.tsx` (inline — only zone still inline) |
