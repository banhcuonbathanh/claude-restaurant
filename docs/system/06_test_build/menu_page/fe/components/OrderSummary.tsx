// P-SYSTEST reference build — Zone I (post-TOP: canh = CartItems via setCanhQty, nhân
// rendered as topping pills, preview == POST payload exactly); not imported by the app.
'use client'
...
// (existing canh items or combo sub-items that include canh). Never hardcoded.
...
export function OrderSummary({ embedded, shakeKey }: { embedded?: boolean; shakeKey?: number }) {
  ...
  // shakeKey bump from the page (blocked checkout) → scroll to + shake the canh block.
  useEffect(() => {
    if (!shakeKey) return
  ...
  // Đơn giá lookup: standalone product prices + combo sub-item unit_price.
  ...
  // "Tổng số món": aggregate by name|toppingKey, EXCLUDE canh, then re-add canh from the
  // canh CartItems — so the on-screen preview matches the POST payload exactly.
  const dishSummary = (() => {
    const map = new Map<string, { name: string; nhan: string; qty: number }>()
    for (const item of items) {
      if (item.type === 'combo' && item.combo_items) {
        for (const ci of item.combo_items) {
          if (isSoupName(ci.product_name)) continue
        ...
        if (isSoupName(item.name)) continue
        const toppingKey = item.toppings.map(t => t.id).sort().join(',')
        const toppingNames = item.toppings.map(t => t.name).join(', ')
    ...
    <section className={embedded ? 'border-t border-border px-5 py-4' : 'mx-4 mt-4 bg-card rounded-xl p-4 shadow-sm mb-4'}>
      ...
      {open && (
        <div className="mt-3 space-y-3">
          ...
          {/* Canh summary — steppers write canh CartItems via setCanhQty */}
          <div
            ref={canhRef}
        ...
        {items.map(item => {
          ...
          return (
            <li key={item.id}>
              ...
              {/* Combo sub-items — edits re-derive the combo line's unit price */}
