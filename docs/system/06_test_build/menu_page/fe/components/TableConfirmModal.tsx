// P-SYSTEST reference build — the page's ONE order-create mutation. Post-TOP contract:
// POST /orders ALWAYS creates; { id, table_busy } — table_busy:true is an info toast,
// not an error (TABLE_HAS_ACTIVE_ORDER no longer exists). Not imported by the app.
'use client'
...
export function TableConfirmModal({ onClose }: { onClose: () => void }) {
  ...
  const submitOrder = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      if (order?.id) {
        // Cache the full order for /order/:id; if the follow-up GET fails the order
        // still succeeded — cache the leaner create response instead.
        try {
