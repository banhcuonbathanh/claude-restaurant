...
|---|---|---|---|---|---|---|---|
| customer_menu | 2026-06-20 (design overlay 2026-06-23; GAP-7 fixed 2026-06-24) | experience_claude.md_system_1_test_iphon2_change_code | 7 | 13 | 10 | [EN](customer/customer_menu/COMPARISON_DOC_VS_CODE_DETAILED.md) · [VI](customer/customer_menu/COMPARISON_DOC_VS_CODE_DETAILED_VI.md) · [Mockup](customer/customer_menu/COMPARISON_VISUAL_MOCKUP_VI.md) | **2026-06-23 DESIGN PIVOT:** doc-set now treats `DESIGN_PROMPT.md` (new design) as spec → **8 new-design rebuild gaps** (Header photo-banner · CategoryTabs scroll-spy · ComboCard multi-select nhân · floating-pill checkout · remove "Gọi thêm" · OrderNote pre-fill · FavouritesRail · worked-example Bàn 04) — FE code still on old design (pending rebuild). **GAP-7 FavouritesRail ✅ resolved (2026-06-24):** component already existed; 3 new-design divergences fixed in `FavouritesRail.tsx` — card tap now opens item detail (`/menu/product|combo/[id]`), heart recolored to `fill-primary` (orange), section label has small Heart icon. Heart on ProductCard/ComboCard/ProductGridCard still red (deferred to GAP-8). **Carried-forward pre-existing issues:** `TableConfirmModal` 201 handoff does **not** call `setActiveOrderId` (doc wrong); `ToppingModal` is dead code (0 imports) — nhân picked via inline pills; `ComboModal` rendered but unreachable; `OrderSummary` far richer than the I-zone ASCII (canh steppers + dish table + save-state note). MenuHeader `tableLabel`-vs-`tableName` blank-after-QR bug is resolved by the header rebuild (pill moves to OrderSummary). |
