...
| 3 | **DELETE is admin-only per spec but button renders for managers** | ❓ UNVERIFIED — the page renders "Xóa" for all logged-in users without a role check; whether the API itself enforces admin-only is not traced here. See `CATEGORIES_BUGS.md` if it exists. |
| 4 | **Client-side sort only** | Rows are sorted by `sort_order` ASC in the browser (`page.tsx:119`: `[...categories].sort((a,b)=>a.sort_order-b.sort_order)`). The `GET /categories` API response order is not guaranteed; no `ORDER BY` clause is verified in this file. |
