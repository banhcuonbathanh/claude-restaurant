...
3. **Category tabs (sticky scroll-spy nav).** A horizontally-scrolling, **sticky** bar of text labels matching the real menu categories: **Tất cả · Suất · Trứng · Bánh Cuốn · Giò · Canh**. These are **navigation anchors, not filters** — every section always renders. Tapping a tab **scrolls** to that section, and scrolling the page **auto-highlights** the tab of the section currently in view. Active style = **orange text + orange underline + a soft orange text-glow**; others muted.
...
6. **Product sections — TRỨNG · BÁNH CUỐN · GIÒ · CANH.** The individual items are grouped under muted uppercase **category section headers** (one section each, in this order). Each product card (full width, rounded, slate background) is the core repeated component:
