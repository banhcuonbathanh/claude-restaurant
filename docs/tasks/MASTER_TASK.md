...
| **FAV — Customer Favourites Redesign (FE only)** | FE | ⬜ DRAFT (awaiting ALIGN) | ~4 | FAV-1 (canh quick-add + live total + drop 2 footer buttons) |
...
> **Status:** ⬜ DRAFT — awaiting owner ALIGN (2026-07-04)
> **Added:** 2026-07-04
...
| FAV-2-FE-1 | FE | **"Tự tạo suất" builder view** — new route `menu/favourites/build/` (View C): grouped steppers (Bánh cuốn/Trứng/Giò/Canh), global "Nhân mặc định (bánh cuốn)" pill group, per-trứng nhân pills + free-text ghi chú (placeholder "vd: nhân để ngoài bánh, ít hành..."), sticky summary bar (`n món · total` live). Local state only — no order/save wiring yet. | FAV-1 | 1 | ⬜ | Builder renders per mockup View C; live total = Σ chosen components; per-egg panel shows/collapses on qty>0/0; `npm run build` green |
| FAV-2-FE-2 | FE | Wire builder → **"Lưu suất này"** save modal → persist suất as **personal data** in `store/favourites.ts` (component recipe: product_id + qty + nhân topping + note); order by **splitting into individual món-lẻ product lines** (both 1-dish and ≥2-dish builds) through the single `lib/order-payload.ts` builder; trứng ghi chú threaded as item **note** (add `note?` to `CartItem` + emit in `order-payload.ts`). No `combos` DB write. | FAV-2-FE-1 | 1 | ⬜ | Ordering a suất produces món-lẻ lines by product_id (total matches Σ, no double-count); trứng notes reach the order for staff; all POSTs go through the one payload builder; nothing written to combos table; builder tests green |
| FAV-4 | FE | **📌 "Ghim lên Menu"** — pin toggle on each saved set (& optionally custom suất) in "Bộ đã lưu"; when pinned the set surfaces as a quick-select card in the menu "YÊU THÍCH" rail for one-tap re-add. Pin state persisted in `store/favourites.ts`. Read `customer_menu` doc-set first (touches the menu rail). Files (est.): `store/favourites.ts` (pin state) · `sets/components/SetCard.tsx` (toggle) · `menu/page.tsx` (rail render + re-add). | FAV-1 | 1 | ⬜ | Pinning a set makes it appear in the menu rail; tapping re-adds the set to cart; unpin removes it; survives reload; `npm run build` green |
